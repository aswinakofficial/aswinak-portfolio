      ...
      // ── Blog posts ───────────────────────────────────────────
      {
        ...
        fields: [
          ...
          { type: 'string',    name: 'slug',         label: 'Slug',         required: true },
          {
            ...
            name: 'readingTime',
            label: 'Reading Time',
            description: 'Custom reading time (e.g. "5 min read" or "10"). If left blank, it will be automatically calculated.',
          },
          {
            type: 'object',
            name: 'photos',
            label: 'Memory Lane Photos',
            list: true,
            ui: {
              itemProps: (item: { caption?: string; location?: string; src?: string }) => ({
                label: item.caption || item.location || item.src || 'Photo',
              }),
            },
            fields: [
              { type: 'image',  name: 'src',       label: 'Photo Image', required: true },
              ...
              { type: 'string', name: 'alt',       label: 'Alt Text' },
              {
                type: 'string',
                name: 'rotation',
                label: 'Rotation (e.g. -3deg, 2.5deg)',
              },
              {
                type: 'string',
                name: 'tapeColor',
                label: 'Tape Color',
                options: ['yellow', 'orange', 'blue', 'green', 'pink', 'purple', 'red', 'cyan', 'black', 'none'],
              },
            ],
          },
